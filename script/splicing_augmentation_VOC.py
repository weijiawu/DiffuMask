...
def splicing_1x2(root,out_root, image_list,n_image=100,start_id=0):
    ...
    for idx in tqdm(range(n_image)):
        ...
        if not os.path.exists("{}/refine_gt_crf/{}".format(root,mask_1)) or not os.path.exists("{}/refine_gt_crf/{}".format(root,mask_2)):
            continue
            
        img1 = cv2.imread("{}/train_image/{}".format(root,image_1))
        img2 = cv2.imread("{}/train_image/{}".format(root,image_2)) 

        mas1 = cv2.imread("{}/refine_gt_crf/{}".format(root,mask_1))
        mas2 = cv2.imread("{}/refine_gt_crf/{}".format(root,mask_2))
        ...
        cv2.imwrite("{}/Image/splicing_{}.jpg".format(out_root,start_id),image)
        cv2.imwrite("{}/Mask/splicing_{}.png".format(out_root,start_id),mask)
...
#     start_id = 0
    ...
    for idx in tqdm(range(n_image)):
        ...
        cv2.imwrite("{}/Image/splicing_{}.jpg".format(out_root,start_id),list_image_ha)
        cv2.imwrite("{}/Mask/splicing_{}.png".format(out_root,start_id),list_mask_ha)
...
def main():
    ...
    image_list = os.listdir('{}/train_image'.format(opt.input_root))
    ...
    for i in range(opt.thread_num):
        ...
        if i == 0:
            ...
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,4000,35001,3))
            p.start()
            processes.append(p)
        elif i == 3:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,3000,55001,5))
            p.start()
            processes.append(p)
        elif i == 4:
            ...
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,2000,75001,7))
            p.start()
            processes.append(p)
        elif i == 6:
