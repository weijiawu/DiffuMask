...
def load_img_name_list(dataset_path,label_la_dir,label_ha_dir):

#     img_gt_name_list = open(dataset_path).read().splitlines()
#     img_name_list = [img_gt_name.split(' ')[0][-15:-4] for img_gt_name in img_gt_name_list]
    if label_la_dir:
        label_la_dir_listr = [i.replace(".npy","") for i in os.listdir(label_la_dir) if "npy" in i]
        label_ha_dir_listr = [i.replace(".npy","") for i in os.listdir(label_ha_dir) if "npy" in i]
    
    img_name_list = [i.replace(".jpg","") for i in os.listdir(dataset_path) if "jpg" in i]
    if label_la_dir:
        img_name_list = [i for i in img_name_list if i in label_la_dir_listr]
        img_name_list = [i for i in img_name_list if i in label_ha_dir_listr]
    
    return img_name_list

class VOC12ImageDataset(Dataset):

    def __init__(self, img_name_list_path,voc12_root,label_la_dir=None, label_ha_dir=None, transform=None):
        self.img_name_list = load_img_name_list(img_name_list_path,label_la_dir,label_ha_dir)
...
class VOC12AffDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, label_la_dir, label_ha_dir, cropsize, voc12_root, radius=5,
        ...
        super().__init__(img_name_list_path,voc12_root, label_la_dir,label_ha_dir, transform=None)
...
#         print(np.unique(label))
        ...
        if self.show:
            print(np.unique(label_la))
            print(np.unique(label_ha))
            
            cv2.imwrite("./output/test/test_label_la{}.jpg".format(name),label_la*255)
            cv2.imwrite("./output/test/test_llabel_ha{}.jpg".format(name),label_ha*255)
